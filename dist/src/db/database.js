...
function initializeDatabase() {
  ...
  `);
    ...
    try {
        exports.db.exec(`ALTER TABLE categories ADD COLUMN icon TEXT`);
    }
    catch (error) {
  ...
  `);
    ...
    try {
        exports.db.exec(`ALTER TABLE vehicles ADD COLUMN status TEXT DEFAULT 'available'`);
    }
    catch (error) {
        // Column already exists, ignore error
    }
  ...
  `);
    // Add show_in_nav column if it doesn't exist (for existing databases)
    try {
        exports.db.exec(`ALTER TABLE pages ADD COLUMN show_in_nav INTEGER DEFAULT 0`);
    }
    catch (error) {
        // Column already exists, ignore error
    }
